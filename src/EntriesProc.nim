import std/[os, posix_utils, strutils, strformat, algorithm]
...
proc Entries*(directory: string = "./", match: string = "", Yield: int = 0): void =
  const
    spaces: int = 10
  ...
  for entry in sorted(entries, Ascending):
    info.add(getFileInfo(entry, followSymlink = false))
