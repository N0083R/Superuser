...
proc Find*(name: string = "", use: string = "/", mode: string = "strict", limit: int = 0, Write: string = ""): void =
  var
  ...
  for entry in walkDirRec(expandTilde(use), yieldFilter={pcFile, pcDir, pcLinkToFile, pcLinkToDir}, followFilter={pcDir}, relative = false, checkdir = false):
    ...
    try:
      ...
      if (mode == "strict" and entryDirName.endsWith(strictDirName)) or (mode == "passive" and entryDirName.contains(passiveDirName)):
        ...
        if Write.len > 0:
          ...
          stdout.writeLine("<", blue, "Directory", reset, "> ", green, "Path", reset, ": ", blue, entry & "/", reset, "\n")
          stdout.flushFile

        if limit > 0 and counter == limit:
          break

      elif (mode == "strict" and entryFileName.endsWith(strictFileName)) or (mode == "passive" and entryFileName.contains(passiveFileName)):
        counter.inc()
