...
import ActionsProc, EntriesProc, EntryCountProc, EntryInfoProc, FindProc, OpenProc, UsageProc
...
proc ProcessArguments*(arguments: seq[string]): void =
  if arguments.len == 0:
    quit(0)
    ...
    if arguments[0].startsWith("entries") and arguments[1].startsWith("match:"):
